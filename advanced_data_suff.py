...
def update_solution_gender(sol, bm_date, slot, insuff, is_wom):
    ...
    sol[i]['slot_details'][j]['gender_insufficiency'][0]['overall_cw_insufficiency'] = max(
        ...
        insuff)
    if is_wom:
        sol[i]['slot_details'][j]['gender_insufficiency'][0]['female_cw_insufficiency'] = max(
            sol[i]['slot_details'][j]['gender_insufficiency'][0]['female_cw_insufficiency'],
            insuff)
    return sol


def update_solution_hard_pref(sol, bm_date, slot, insuff, is_wom):
    """updates the insufficiency list when the insufficiency is detected
    returns the updated insufficiency list"""
    # get index of the dict for the date having insufficiency if present
    # if not then append the date with corresponding slot, skill, and set insufficiency to 0
    ...
    # set index to that of the last element
    # set insufficiency as max calculated insufficiency and if woman su, add woman insufficiency
    sol[i]['slot_details'][j]['hard_pref_failure_insufficiency'][0]['overall_pref_insufficiency'] = max(
        sol[i]['slot_details'][j]['hard_pref_failure_insufficiency'][0]['overall_pref_insufficiency'],
        insuff)
    if is_wom:
        sol[i]['slot_details'][j]['hard_pref_failure_insufficiency'][0]['female_pref_insufficiency'] = max(
            sol[i]['slot_details'][j]['hard_pref_failure_insufficiency'][0]['female_pref_insufficiency'],
            insuff)
    return sol
    ...
    for date in dates_in_range:
        ...
        for booking in bm_master:
            ...
            for slot in hours:
                ...
                # advanced insufficiency
                cw_hard_pref = advanced_sufficiency(cw, booking)
                total_cw_hard_pref = len(cw_hard_pref.distinct('user_id'))
                # check insufficiency, if present, update hard pref solution
                if num_cw_req_in_slot > total_cw_hard_pref:
                    insuff_hard_pref = num_cw_req_in_slot - total_cw_hard_pref
                    sol = update_solution_hard_pref(sol, bm_date_in_dict, slot, insuff_hard_pref, is_wom)
